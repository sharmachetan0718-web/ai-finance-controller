...
# ============================================================
# SIMPLE DEMO AUTHENTICATION + USER DATA STORAGE
# ============================================================
AUTH_FILE = "users_data.json"
...
def _create_session(username):
    """Create a persistent browser session token for this user."""
    users = _read_users()
...
def _get_user_from_session():
    """Restore the signed-in user after a browser refresh, if the token is valid."""
...
def load_user_data(username):
    users = _read_users()
    user = users.get(username, _empty_user_data())
    return (
        user.get("custom_payments", []),
        user.get("custom_settlements", []),
        user.get("audit_log", []),
        user.get("transaction_overrides", {}),
        user.get("deleted_payment_ids", [])
    )
...
def save_current_user_data():
    ...
    users = _read_users()
    user = users.get(username, _empty_user_data())
    user["custom_payments"] = st.session_state.get("custom_payments", [])
    user["custom_settlements"] = st.session_state.get("custom_settlements", [])
    user["audit_log"] = st.session_state.get("audit_log", [])
    user["transaction_overrides"] = st.session_state.get("transaction_overrides", {})
    user["deleted_payment_ids"] = st.session_state.get("deleted_payment_ids", [])
    users[username] = user
    _write_users(users)
...
def show_login():
    st.markdown(
        """
        <div style="max-width:520px;margin:70px auto 0 auto;text-align:center;">
            <div style="font-size:2rem;font-weight:750;color:#18202a;">Finance Operations</div>
    ...
    login_col, _ = st.columns([1, 0.25])
    with login_col:
        tab_login, tab_signup = st.tabs(["Sign in", "Create account"])

        with tab_login:
            st.markdown("### Welcome back")
            st.caption("Sign in to access your saved transactions and investigations.")
            with st.form("login_form"):
                username = st.text_input("Username", placeholder="e.g. chetan")
            ...
            if submitted:
                ...
                if username in users and users[username].get("password_hash") == _hash_password(password):
                    ...
                    (
                        ...
                        st.session_state.transaction_overrides,
                        st.session_state.deleted_payment_ids
        ...
        with tab_signup:
            ...
            st.caption("Your dashboard data will be stored separately for this account.")
            with st.form("signup_form"):
                new_username = st.text_input("Choose username", placeholder="e.g. chetan")
            ...
            if create_account:
                ...
                if len(new_username) < 3:
                    ...
                    st.error("Use only letters, numbers, and underscores in the username.")
                elif not new_password:
                    st.error("Password is required.")
                elif new_password != confirm_password:
                    st.error("Passwords do not match.")
                elif new_username in users:
                    st.error("That username already exists. Please sign in.")
                else:
                    user = _empty_user_data()
...
/* Highlight only the new top navigation card. */
div[data-testid="stVerticalBlockBorderWrapper"]:has(.finance-header-marker) {
    background: #ffffff !important;
    border: 1px solid #cfd8e3 !important;
    border-radius: 14px !important;
    box-shadow: 0 3px 12px rgba(16, 42, 67, 0.08) !important;
    padding: 6px 8px !important;
    margin-bottom: 28px !important;
}

.finance-header-marker {
    height: 0;
    overflow: hidden;
    margin: 0;
    padding: 0;
...
# Restore the persistent browser session after a refresh.
if not st.session_state.logged_in:
    restored_user = _get_user_from_session()
    if restored_user:
        st.session_state.logged_in = True
        st.session_state.current_user = restored_user

# Require sign-in before loading or showing finance data.
if not st.session_state.logged_in:
    show_login()
...
if "transaction_overrides" not in st.session_state:
    st.session_state.transaction_overrides = {}

if "deleted_payment_ids" not in st.session_state:
    st.session_state.deleted_payment_ids = []
...
# Apply persistent transaction edits/deletions before reconciliation.
deleted_ids = {str(x) for x in st.session_state.deleted_payment_ids}
if deleted_ids:
    payments = payments[~payments["payment_id"].astype(str).isin(deleted_ids)].copy()
    settlements = settlements[~settlements["payment_id"].astype(str).isin(deleted_ids)].copy()

for override_id, override in st.session_state.transaction_overrides.items():
    override_id = str(override_id)
    if override_id in deleted_ids:
        continue
    payment_mask = payments["payment_id"].astype(str) == override_id
    if payment_mask.any():
        if "merchant_id" in override:
            payments.loc[payment_mask, "merchant_id"] = override["merchant_id"]
        if "amount" in override:
            payments.loc[payment_mask, "amount"] = float(override["amount"])
        # Rebuild this transaction's settlements from the saved edit.
        settlements = settlements[settlements["payment_id"].astype(str) != override_id].copy()
        count = int(override.get("settlement_count", 0))
        settled_amount = float(override.get("settled_amount", 0.0))
        today = datetime.now().strftime("%Y-%m-%d")
        if count > 0:
            next_id = len(settlements) + 1
            new_rows = []
            for i in range(count):
                new_rows.append({
                    "settlement_id": f"SE{next_id + i:04d}",
                    "payment_id": override_id,
                    "settled_amount": settled_amount,
                    "fee": 0.0,
                    "settlement_date": today,
                    "settlement_status": "SETTLED"
                })
            settlements = pd.concat([settlements, pd.DataFrame(new_rows)], ignore_index=True)
...
with st.container(border=True):
    st.markdown('<div class="finance-header-marker"></div>', unsafe_allow_html=True)

    header_left, header_nav, header_right = st.columns(
        [2.7, 4.8, 2.5],
    ...
    with header_left:
        st.markdown(
            """<div class="finance-brand">Finance Operations</div>
            <div class="finance-brand-sub">Payment matching &amp; exception review</div>""",
            unsafe_allow_html=True
        )
    ...
    with header_nav:
        st.markdown(
            ...
            <div class="header-nav">
                ...
                <a href="#exception-queue">Exceptions</a>
                <a href="#transaction-review">Transactions</a>
    ...
    with header_right:
        status_col, add_col, menu_col = st.columns(
            [1.05, 1.55, 0.45],
            vertical_alignment="center"
        )
        ...
        with status_col:
            st.markdown(
                '<div class="header-status"><span class="header-status-dot"></span>Ready</div>',
                unsafe_allow_html=True
            )
        ...
        with add_col:
            with st.popover("＋ Add transaction", use_container_width=True):
                ...
                st.caption("Enter a new payment to test the reconciliation engine.")
        ...
        with menu_col:
            with st.popover("⋮", use_container_width=True):
                ...
                if st.button("Log out", use_container_width=True):
                    save_current_user_data()
                    _clear_session(st.session_state.current_user)
                    st.session_state.logged_in = False
                    st.session_state.current_user = None
                    st.session_state.user_data_loaded = False
                    st.session_state.payments = None
...
st.title("Reconciliation Control Center")
...
st.markdown('<div class="section-note">Select any transaction to edit its details or remove it from this workspace.</div>', unsafe_allow_html=True)

# Prominent transaction controls: always visible and directly above the
# edit/delete form so users do not have to hunt for the actions.
management_options = reconciliation["payment_id"].astype(str).tolist()
if management_options:
    manage_payment_id = st.selectbox(
        "Select Transaction",
        management_options,
        key="transaction_management_selector"
    )
    manage_row = reconciliation[reconciliation["payment_id"].astype(str) == str(manage_payment_id)].iloc[0]

    action_col1, action_col2 = st.columns(2)
    with action_col1:
        edit_clicked = st.button(
            "✏️ Edit Transaction",
            key="edit_transaction_button",
            use_container_width=True
        )
    with action_col2:
    ...
    # Store the chosen action in session state so the controls remain open
    # after Streamlit reruns.
    if edit_clicked:
        st.session_state.transaction_action = "Edit transaction"
    elif delete_clicked:
        st.session_state.transaction_action = "Delete transaction"

    transaction_action = st.session_state.get("transaction_action", "Select action")

    if transaction_action == "Edit transaction":
        with st.form("edit_transaction_form"):
            edit_col1, edit_col2, edit_col3 = st.columns(3)
            with edit_col1:
                edit_merchant_id = st.text_input("Merchant ID", value=str(manage_row["merchant_id"]))
                edit_amount = st.number_input("Payment Amount (₹)", min_value=0.0, value=float(manage_row["amount"]), step=100.0)
            with edit_col2:
                edit_settled_amount = st.number_input("Settled Amount (₹)", min_value=0.0, value=float(manage_row["settled_amount"]), step=100.0)
                edit_settlement_count = st.number_input("Settlement Count", min_value=0, value=int(manage_row["settlement_count"]), step=1)
            with edit_col3:
                edit_payment_id = st.text_input(
                    "Payment ID",
                    value=str(manage_payment_id)
                )
                st.caption("Payment ID can be changed. It must remain unique.")

            save_edit = st.form_submit_button("Save Changes", use_container_width=True)

            if save_edit:
                old_payment_id = str(manage_payment_id)
                new_payment_id = edit_payment_id.strip()

                existing_ids = set(payments["payment_id"].astype(str).tolist())
                if new_payment_id == "":
                    st.error("Payment ID cannot be empty.")
                elif new_payment_id != old_payment_id and new_payment_id in existing_ids:
                    st.error(f"Payment ID {new_payment_id} already exists. Please choose a unique ID.")
                else:
                    # If the ID changes, move the saved override to the new ID
                    # and update the underlying payment/settlement records.
                    if new_payment_id != old_payment_id:
                        payment_mask = st.session_state.payments["payment_id"].astype(str) == old_payment_id
                        st.session_state.payments.loc[payment_mask, "payment_id"] = new_payment_id

                        settlement_mask = st.session_state.settlements["payment_id"].astype(str) == old_payment_id
                        st.session_state.settlements.loc[settlement_mask, "payment_id"] = new_payment_id

                        if old_payment_id in st.session_state.transaction_overrides:
                            st.session_state.transaction_overrides[new_payment_id] = (
                                st.session_state.transaction_overrides.pop(old_payment_id)
                            )

                        if old_payment_id in [str(x) for x in st.session_state.deleted_payment_ids]:
                            st.session_state.deleted_payment_ids = [
                                new_payment_id if str(x) == old_payment_id else x
                                for x in st.session_state.deleted_payment_ids
                            ]

                    st.session_state.transaction_overrides[new_payment_id] = {
                        "merchant_id": edit_merchant_id.strip(),
                        "amount": float(edit_amount),
                        "settled_amount": float(edit_settled_amount),
                        "settlement_count": int(edit_settlement_count)
                    }
                    st.session_state.audit_log.append({
                        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                        ...
                        "severity": "INFO",
                        "action": (
                            f"Transaction updated through dashboard input."
                            + (f" Payment ID changed from {old_payment_id} to {new_payment_id}."
                               if new_payment_id != old_payment_id else "")
                        )
                    })
                    save_current_user_data()
                    st.success(f"{new_payment_id} updated successfully.")
                    st.session_state.transaction_action = "Select action"
                    st.rerun()
                save_current_user_data()
                st.success(f"{manage_payment_id} updated successfully.")
                st.session_state.transaction_action = "Select action"
                st.rerun()

    elif transaction_action == "Delete transaction":
        st.warning(f"You are about to delete transaction **{manage_payment_id}**. This will remove it from reconciliation and exception views.")
        confirm_delete = st.checkbox("I understand this transaction will be removed from this workspace.", key="confirm_delete_transaction")
        if st.button("Delete Transaction", type="primary", use_container_width=True, disabled=not confirm_delete):
            payment_id = str(manage_payment_id)
            st.session_state.deleted_payment_ids.append(payment_id)
            st.session_state.transaction_overrides.pop(payment_id, None)
            st.session_state.audit_log.append({
                "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "type": "TRANSACTION DELETED",
                "payment_id": payment_id,
                "severity": "WARNING",
                "action": "Transaction deleted through dashboard input."
            })
            save_current_user_data()
            st.success(f"{payment_id} deleted successfully.")
            st.session_state.transaction_action = "Select action"
            st.rerun()
